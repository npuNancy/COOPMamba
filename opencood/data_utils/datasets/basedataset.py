...
class BaseDataset(Dataset):
    ...
    def add_random_noise(self, pose):
        np.random.seed(self.seed)
        xyz_noise = np.random.uniform(-0.1,0.1,3)
        ryp_std = np.random.uniform(-0.1,0.1)
        noise_pose = [pose[0] + xyz_noise[0],
                      pose[1] + xyz_noise[1],
                      pose[2] + xyz_noise[2],
                      pose[3],
    ...
    def reform_param(self, cav_content, ego_content, timestamp_cur,
        ...
        if not cav_content['ego'] :
            delay_cav_lidar_pose = self.add_random_noise(delay_cav_lidar_pose)
            cur_cav_lidar_pose = self.add_random_noise(cur_cav_lidar_pose)

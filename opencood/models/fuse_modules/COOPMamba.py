...
class COOPMamba(nn.Module):
    ...
    def forward(self,data,record_len):
        ...
        out = []
        for xx in split_x:
            xmax = torch.max(xx, dim=0, keepdim=True)[0]
            xaverage =torch.mean(xx,dim=0,keepdim=True)
            xmax = rearrange(xmax, 'b c h w ->b (h w) c', h=h, w=w)
            xaverage= rearrange(xaverage, 'b c h w ->b (h w) c', h=h, w=w)
            for sp1_layer, sp2_layer in zip(self.sp1_mamba_layers, self.sp2_mamba_layers):
                 xmax =sp1_layer(xmax)
                 xaverage=sp2_layer(xaverage)
            sp1_fusion =self.sp1_cross_mamba(xmax,xaverage)
            sp2_fusion =self.sp2_cross_mamba(xaverage,xmax)
            fusion = self.out_proj((sp1_fusion+sp2_fusion)/2)
